...
def visual(node_fea, label):
    ...
    save_path = '/mnt/cpath2/lf/111.png'
    tsne = TSNE(random_state = 42, n_components=2,verbose=0, perplexity=40, n_iter=300).fit_transform(node_fea)
    plt.scatter(tsne[:, 0], tsne[:, 1], s= 5, c=label, cmap='Spectral')
    plt.gca().set_aspect('equal', 'datalim')
    plt.colorbar(boundaries=np.arange(11)-0.5).set_ticks(np.arange(10))
    plt.title('Visualizing WSI_preprocessign through t-SNE', fontsize=24)
...
def testt(dataloader_tset, model, args):
    ...
    visual(feature_vector, labels_vector)
