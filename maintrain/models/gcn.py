...
class GraphConv(nn.Module):
    ...
    #             print(graph.edata)
            ...
            graph.srcdata['h'] = node_fea
            graph.update_all(aggregate_fn, fn.mean(msg='m', out='h'))
            print(f"更新后node_fea.size(){graph.ndata['h'].size()}")
            graph.apply_edges(fn.u_add_v('h', 'h', 'e'))  # TODO相似度  
            rst = graph.dstdata['h']
            ...
            print(f"sizes {rst.size()}, {graph.edata['e'].size()}")
...
if __name__ =='__main__':
    graph_model = GCN(in_dim=768, num_hidden=768, out_dim=768, num_layers=40, dropout=0,activation="prelu", residual=True,norm=nn.LayerNorm).to('cuda:0')
    optimizer = torch.optim.SGD(graph_model.parameters(), lr=1e-5)
    loss_fn = torch.nn.MSELoss().to("cuda:0")
    g = dgl.rand_graph(120000, 100000).to("cuda:0")
    node_fea = torch.randn(120000, 768).to('cuda:0')
    edge_fea = torch.randn(100000, 1).to('cuda:0')
    a = graph_model(g, node_fea, edge_fea)
    label = torch.randn(120000, 768).to("cuda:0")
    ...
    kk = get_parameter_number(graph_model)
    print(kk)
